...
def particle_swarm_optimization(func, iterations, n_particles, w, c1, c2, x_bounds, y_bounds, animation_speed,
                                generate_gif):
    ...
    anim_pso = FuncAnimation(fig_contour, animate, frames=iterations, interval=animation_speed, blit=False)
...
def run_pso():
    try:
        ...
        animation_speed = int(speed_entry.get())
        generate_gif = gif_var.get()
        particle_swarm_optimization(func, iterations, n_particles, w, c1, c2, x_bounds, y_bounds, animation_speed,
                                    generate_gif)
...
tk.Label(root, text="Number of iterations:").grid(row=0, column=0, sticky="e")
iterations_entry = tk.Entry(root)
iterations_entry.insert(0, "50")
iterations_entry.grid(row=0, column=1)

tk.Label(root, text="Number of particles:").grid(row=1, column=0, sticky="e")
particles_entry = tk.Entry(root)
particles_entry.insert(0, "30")
particles_entry.grid(row=1, column=1)

tk.Label(root, text="Inertia coefficient (w):").grid(row=2, column=0, sticky="e")
w_entry = tk.Entry(root)
w_entry.insert(0, "0.8")
w_entry.grid(row=2, column=1)

tk.Label(root, text="Cognitive coefficient (c1):").grid(row=3, column=0, sticky="e")
c1_entry = tk.Entry(root)
c1_entry.insert(0, "0.1")
c1_entry.grid(row=3, column=1)

tk.Label(root, text="Social coefficient (c2):").grid(row=4, column=0, sticky="e")
...
tk.Label(root, text="Animation speed (ms per frame)").grid(row=9, column=0, sticky="e")
speed_entry = tk.Entry(root)
speed_entry.grid(row=9, column=1)
speed_entry.insert(0, "500")

tk.Label(root, text="Function:").grid(row=10, column=0, sticky="e")
func_selector = ttk.Combobox(root, values=list(objective_functions.keys()), state="readonly")
func_selector.grid(row=10, column=1)
func_selector.current(0)

gif_var = tk.BooleanVar()
gif_checkbox = tk.Checkbutton(root, text="Generate GIF", variable=gif_var)
gif_checkbox.grid(row=11, column=1, sticky="w")

run_button = tk.Button(root, text="Run PSO", command=run_pso)
run_button.grid(row=12, column=0, columnspan=2)
